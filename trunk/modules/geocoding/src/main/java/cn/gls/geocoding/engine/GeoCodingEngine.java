...
public class GeoCodingEngine implements IGeoCodingEngine {
	...
	public GeoCodingResponse getResponse(GeoCodingRequestParameter gParam,
		...
		List<Place> places = segEngine.standardPlace(
				segAddress.getPlacesList(), citycode);

		// 进入辅助引擎@zhangdp 2012-11-29，返回辅助引擎的结果
		// 如果有拼音或错字则返回地址
		if (segAddress.isMisprint() || segAddress.isPinyin()) {
			// 返回正确的地址
			String flag = "Message.Pinyin";
			if (segAddress.isMisprint())
				flag = "Message.Error";
			response.setMessage(context.getProperties().getProperty(flag));
			List<Result> results = new ArrayList<Result>();

			if (segEngine.getAddresses() != null) {
				List<SegAddress> addresses = segEngine.getAddresses();
				for (SegAddress address : addresses) {
					GeoCodingResult result = new GeoCodingResult();
					StandardAddress st_address = new StandardAddress();
					// 构建标准地址
					st_address.setSt_address(StringUtils
							.getResultString(address.getPlacesList()));
					result.setAddress(st_address);
					results.add(result);
				}
	...
	public List<GeoCodingResponse> getGeoCodingBatchResults(
		...
		GeoCodingResponse response = new GeoCodingResponse(request);
		GeoCodingRequestParameter gParam = request.getGbsRequestParameter();
		// List<String> qs = Arrays.asList(gParam.getQ().split("/n"));
